...
3. **Le schéma vit dans `supabase/migrations/`.** Aucune modification manuelle en prod. Après une migration : `pnpm db:types` et commit du type généré. Mettre à jour `docs/DATA-MODEL.md` si le schéma s'écarte du document. Les skills `.claude/skills/supabase*` sont **consultatives** et ne priment jamais sur cette règle ni sur la règle 2 : leur procédure « imperative migrations » (`execute_sql` pour itérer, puis `supabase db pull`) ne vaut que pour une base **locale** — le MCP pointant sur le projet de production, ne jamais l'y appliquer.
