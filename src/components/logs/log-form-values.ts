...
export type LogFormValues = {
  id: string;
  title: string;
  /** Every system it touches, the principal first (D118). */
  categoryIds: string[];
 ...
 * or the reading of the document the intervention starts from (D119).
 */
export type LogFormPrefill = {
  ...
  cost?: string;
  notes?: string;
  /** The provider as the document spells it, when no contact of the boat matched (D120). */
 ...
 * carries several systems since D118 and « Refaire » must bring them all back.
 */
...
/**
 * The document an intervention starts from (D119): it is already in the inbox — same upload,
 ...
 * sheet all arrive knowing; only the bare « + » opens on the document (D119).
 */
