...
export async function saveLog(input: unknown): Promise<ActionResult<SavedLog>> {
  ...
  const row = {
    ...
    boat_id: boatId,
    title: values.title,
    // The principal stays in its own column (D114): every filter, the report and the export
...
export async function suggestChecklistItems(
  ...
  const supabase = await createClient();
  // One call per system (D114): the function answers for one, and an intervention that carries
