-- 0035_queue_waits.sql — E18-2 / D122: the queue says everything that waits for someone.
--
...
as $$
  with queue as (
    ...
    -- rank 2: documents waiting for a decision (D91, D122). A paper waits for a person, never
    ...
    -- rank 5: parts at or under their threshold (D10, D122). No date either: this one falls when
...
comment on function public.boat_todo_queue(uuid, int) is
  'Dashboard queue (D122): urgent logs (0), overdue items by relative delay (1), documents waiting on « À valider » oldest first (2), in-progress then planned logs within 30 days (3), items due soon (4), parts under their threshold, shortest stock first (5). Items with status never are excluded on purpose (audit §3.5).';
...
comment on view public.boat_dashboard_stats is
  'What the dashboard banner still asks the database for: how many imported rows wait for a check (D122). Everything else this view carried left with the summary blocks (E18-1).';
