...
test.describe("§6.2 oil change at the dock", () => {
  ...
  test("notes an intervention within the tap budget", async ({ page, request }) => {
    ...
    await expect(page.getByRole("heading", { name: fr.attachments.title })).toBeVisible();
    await expect(page.getByLabel(fr.logs.form.title)).toBeVisible();

    // Focus is explicit: the form leaves the document controls visible on entry.
    await taps.tap(page.getByLabel(fr.logs.form.title));
    await page.getByLabel(fr.logs.form.title).fill("Vidange moteur SB");

    // 3 — the category, as a chip rather than a select (rule 13). A checkbox since D118: an
    ...
    // 4 — save. Date, status and engine hours are already carrying their defaults.
