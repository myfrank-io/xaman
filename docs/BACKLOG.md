...
- [x] **E0-9 (M, 2)** **Un déploiement ne transporte que ce qu'il ouvre** (D139). 9,92 Go de Functions Storage et 6,78 Go de Deployment Storage sur le compte, dont 5,4 Go pris en une seule journée. Mesuré sur les `*.nft.json`, ce que Vercel recopie dans chaque lambda : les deux routes qui savent lire un document pesaient 61,3 et 59,7 Mo, dont **44 Mo par route qu'aucun runtime Node ne peut ouvrir** — les six cœurs OCR non-LSTM (l'appel est en OEM 1), les trois cœurs LSTM en build navigateur (Node lit le `.js` et son `.wasm`), la source map de pdf.js (5,2 Mo, plus grosse que le code qu'elle décrit), `zlibjs` (le gunzip du navigateur, quand Node a le sien) et le modèle français recopié deux fois. **Fait** : le modèle part dans le bucket public `ocr-assets` (`0041`) lu par URL et mis en cache dans `/tmp` ; les cœurs morts quittent `outputFileTracingIncludes` ; `scripts/trim-server-trace.mjs` retire après le build ce que le tracer ajoute seul, `outputFileTracingExcludes` étant inopérant sous Turbopack ; `vercel.json` gagne un `ignoreCommand` qui ne construit plus les commits de documentation ; `scripts/vercel-prune-deployments.mjs` efface les déploiements passés, la seule chose qui fasse baisser un compteur déjà rempli. **Mesure** : 61,3 → **17,3 Mo** et 59,7 → **15,7 Mo** par route, 70,4 → **26,4 Mo** pour l'union de toutes les routes. **DoD** : mesure avant/après sur les fichiers de tracing, test unitaire de la règle de coupe dans les deux sens, tests RLS du nouveau bucket (lecture pour tous, écriture pour personne), `DATA-MODEL.md` à jour.
...
- [~] **E4-14 (M, 3)** Plusieurs catégories par point, photos et documents dès la création, formulaires checklist/intervention alignés (D150). DoD : classement unique dans tous les filtres, pièces jointes persistantes, reprise après échec, RLS et parcours testés, vérification iPad paysage/portrait.
