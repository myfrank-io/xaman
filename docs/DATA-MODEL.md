...
| tracks_hours | boolean | not null default true | false = pas de compteur d'heures (annexe, hors-bord — D71) : aucun relevé n'est demandé, les échéances en heures de ses points sont neutralisées par `checklist_item_status` |
...
| engine_hours | numeric(8,1) | null | heures du moteur lié au moment du cochage. **Obligatoire si le point a un `interval_hours`** (validation zod + trigger `check_completion_hours`) — sauf sur un moteur sans compteur (`engines.tracks_hours = false`, D71). Crée un `engine_hour_readings` (source 'checklist') sauf si `maintenance_log_id` est renseigné (l'intervention porte déjà ses relevés) |
| note | text | | |
...
--                                    (engines.tracks_hours = false, D71).
