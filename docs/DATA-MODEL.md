...
| Colonne | Type | Contraintes | Notes |
|---|---|---|---|
| log_id | uuid | FK maintenance_logs on delete cascade, PK avec `category_id` | |
| category_id | uuid | FK boat_categories on delete cascade | |
...
| part_id | uuid | FK parts on delete set null, null | **la pièce que cet achat remet au stock** (D143, `0045`) : « Racheté » sur la liste « À racheter », ou tout achat qui nomme une pièce. Même bateau que l'achat, vérifié par le trigger `purchases_part_boat` (`part_boat_mismatch`, `23514`). Les unités de `quantity` entrent au stock par `apply_purchase_to_stock()` ; null sur un achat qui n'est pas du stock qui rentre |
| notes | text | | |
| needs_review | boolean | not null default false | import carnet papier |
| external_ref | text | | |
...
| min_quantity | numeric(8,2) | not null default 0 | ce qu'on veut garder en réserve ; la pièce est « à racheter » si `quantity < min_quantity` et `min_quantity > 0` (**strictement**, depuis D143 / `0045` : tenir exactement son seuil n'est pas un manque, et « Racheté », qui achète ce qui manque, doit vider la ligne) |
...
Suppression : mise à la corbeille (`deleted_at`), restaurable 30 jours. `adjust_part_quantity()` refuse une ligne à la corbeille (`part_not_found`), et la file (`boat_todo_queue`, rang 5) ne la demande plus.
...
- Storage bucket `ocr-assets` (**public**, `0041`, D139) : le modèle OCR français `fra.traineddata.gz`, et rien d'autre. Lecture ouverte à `anon` et `authenticated` (politique `ocr_assets_read`), **aucune** politique d'écriture — donc seul le rôle de service écrit, depuis `pnpm ocr:push`. Public à dessein : le worker Tesseract va chercher le modèle avec un `fetch` nu, sans session où mettre un jeton, et une URL signée expirerait ; ce qu'il contient est de la donnée Apache-2.0 publiée par Google, pas un octet sur un bateau. Le seul bucket public du projet — les documents des bateaux restent dans `boat-files`, privé.
...
`category_color`, `amount`, `hours`, `recorded_at`. `who` prend le **nom figé** d'un cochage
...
| 5 | pièces **strictement** sous leur seuil, corbeille exclue (D10, D131, D143 / `0045`) | `-severity` — ce qui manque (`min_quantity − quantity`), le stock le plus court d'abord |
...
- **0045** (D143) : **un achat de pièce est du stock qui rentre.** `purchases_check_part_boat()` (trigger `purchases_part_boat`, avant insert et update de `part_id` / `boat_id`) refuse une pièce d'un autre bateau (`part_boat_mismatch`, `23514`, lu sous la RLS de l'appelant). `apply_purchase_to_stock()` — trigger `apply_purchase_to_stock`, après insert, après update de `part_id`, `quantity` ou `deleted_at`, et après delete — applique au stock **la différence** que l'écriture fait : écrit → les unités entrent, mis à la corbeille → elles ressortent, restauré → elles rentrent, recompté ou repointé → seul l'écart bouge. Un achat à la corbeille n'apporte rien, donc « Annuler » rend les unités. Le mouvement passe par `apply_stock_movement(p_part_id, p_delta)` (`quantity = greatest(0, quantity + delta)`, `checked_at = current_date`) ; `security definer` sur le trigger, comme `sync_log_completion` (D140), pour qu'un mouvement ne puisse jamais échouer en silence ; `EXECUTE` retiré aux trois fonctions pour `anon` et `authenticated` (`0009`, `0043`). Le stock reste une quantité **comptée** — on consomme une pièce sans rien acheter — ce qu'il cesse d'être, c'est un second endroit où saisir ce qu'une facture dit déjà. Index `purchases_part_idx`. La même migration fait lire au rang 5 de `boat_todo_queue` le seuil **strictement** (voir §12.5).
...
- **0041** (D139) : bucket public `ocr-assets` et sa politique de lecture. Le modèle OCR quitte le dépôt déployé : il était tracé dans les deux fonctions capables de lire un document, donc recopié deux fois par déploiement, pour un fichier qui change une fois l'an. `src/lib/inbox/extract.ts` le lit désormais par URL (`langPath`), la copie du dépôt reste la source. Aucune table, aucune fonction.
