...
Chaque intervention contient : **titre** (ex. « Vidange moteur SB »), **catégorie** (les systèmes du bateau ; au moins un, obligatoire à la saisie — plusieurs depuis D116, le premier coché restant le principal que porte `category_id`, nullable en base pour l'import), **statut** (Planifié / En cours / Terminé / Urgent), **priorité** (Basse / Normale / Haute), **date de l'intervention**, **prochaine échéance** (date, optionnelle), **heures moteur** au moment de l'intervention (un champ par moteur, optionnel — le remplir crée automatiquement un relevé d'heures), **coût** (€), **prestataire** (choisi dans l'annuaire, ou « nous-mêmes »), **notes libres**, et (Should) photos / pièces jointes.
