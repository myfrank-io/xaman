...
- Première mise en route : Joseph (admin plateforme) exécute le seed, qui **crée les comptes** de Xavier (`owner`), Emmanuel (`editor`) et Joseph (`editor` + admin plateforme) via l'API admin Supabase (chacun reçoit un e-mail d'invitation Supabase) et les inscrit directement comme membres du bateau. Aucune page publique « créer mon bateau » en V1 ; la création de bateau est réservée à l'admin plateforme (à ouvrir en V2). L'inscription Supabase Auth reste techniquement ouverte, car indispensable au parcours d'invitation ; un compte sans bateau ne voit qu'une page d'attente.
...
- Création libre d'un bateau par un nouvel utilisateur (onboarding public) et choix du modèle de checklist.
- Modèle de checklist générique (voilier / moteur) complet.
