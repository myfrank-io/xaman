...
- ~~Création libre d'un bateau par un nouvel utilisateur (onboarding public) et choix du modèle de checklist.~~ **Livré (D64, D65, D67, D68)** : `/boats/new` (identité seule), `create_boat` (`0015` + `0017` + `0021`), choix du plan à l'étape 3 de la mise en route, suggestions issues du catalogue `boat_models`.
