...
| Marché (D122) | **Deux acheteurs, un seul produit** : le propriétaire ne paie pas (partage illimité, export toujours gratuit), le **constructeur** vend une option de service avec le bateau et est le cœur de cible ; seul moment payant côté propriétaire, la passation à la vente. Le site public (`/` et `/constructeurs`) porte les deux lectures (D123) |
...
7. **Les prix du marché plafonnent l'ambition tarifaire côté plaisancier** : < 30 €/an (Ready4Sea), 49 €/an (Boatwise), 60 €/an (BoatOn). **D122 en tire la conséquence inverse de celle attendue** : plutôt que de se glisser dans cette fourchette, Xaman n'y vend rien. Le propriétaire ne paie pas, le partage n'est jamais facturé, et la valeur se facture là où un bon de commande est déjà ouvert — chez le constructeur. Le seul encaissement côté propriétaire est la **passation** à la vente (le carnet vivant remis à l'acheteur, le privé du vendeur filtré, un certificat de passation), jamais l'export, qui reste gratuit et affiché.
...
| Constructeur / chantier | **Promu cœur de cible par D122** — voir §4.4 | `organizations` (type `builder`), `checklist_templates.owner_organization_id`, `boats.organization_id` |
...
### 4.4 Le constructeur (cœur de cible, D122 — hors V1, épique E19)
...
#### M2. Fiche bateau
- Identité : nom, constructeur, modèle, numéro de coque, **immatriculation** (D69 — saisie à la main : aucun registre ne la convertit en caractéristiques), année, type (catamaran, monocoque, moteur…), pavillon, port d'attache, longueur/largeur/tirant d'eau, photo.
...
Chaque intervention contient : **titre** (ex. « Vidange moteur SB »), **catégorie** (les systèmes du bateau ; au moins un, obligatoire à la saisie — plusieurs depuis D118, le premier coché restant le principal que porte `category_id`, nullable en base pour l'import), **statut** (Planifié / En cours / Terminé / Urgent), **priorité** (Basse / Normale / Haute), **date de l'intervention**, **prochaine échéance** (date, optionnelle), **heures moteur** au moment de l'intervention (un champ par moteur, optionnel — le remplir crée automatiquement un relevé d'heures), **coût** (€), **prestataire** (choisi dans l'annuaire, ou « nous-mêmes »), **notes libres**, et (Should) photos / pièces jointes.
...
- ~~Création libre d'un bateau par un nouvel utilisateur (onboarding public) et choix du modèle de checklist.~~ **Livré (D64, D65, D67, D69)** : `/boats/new` (identité seule), `create_boat` (`0015` + `0017` + `0021`), choix du plan à l'étape 3 de la mise en route, suggestions issues du catalogue `boat_models`.
...
- UI organisations / flottes / loueurs / locataires / clubs (modèle prévu, voir §4.2). **L'étage constructeur en sort** (D122) : il n'entre pas en V1 non plus, mais il cesse d'être un « un jour peut-être » — il est l'épique **E19**, et E18-9 à E18-12 en sont le socle.
- Inventaire avancé de pièces (emplacements multiples, codes-barres, décrément automatique).
- ~~Facturation / abonnement / paiement.~~ **Requalifié par D122** : hors V1, mais dans le périmètre du produit — contrat facturé pour le chantier, paiement unique pour la passation à la vente (E19). Le carnet du propriétaire, lui, ne rencontre jamais de mur de paiement.
