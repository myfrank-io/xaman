...
const INBOX_READER_FILES = [
  ...
  // The OCR engine, every build: the worker picks one by the CPU's wasm features and the OCR mode
  // (Vercel's runtime asked for `tesseract-core-relaxedsimd`, which an LSTM-only list left out).
