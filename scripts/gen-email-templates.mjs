...
/**
 * The invitation, a second time, as a TypeScript string. The app sends its own invitation when a
 * mailer is configured (D75) — for the people Supabase refuses to invite because they already
 * have an account — and it must be the same e-mail, to the pixel. Reading the .html at runtime
 * would mean carrying a file into the serverless bundle and hoping the tracer agrees; a
 * generated module is simply imported, and the drift test covers it like the rest.
 */
export const APP_TEMPLATE = path.join(root, "src", "lib", "email", "invitation.generated.ts");
...
export const TEMPLATES = [
  ...
  },
  {
    ...
    body: [
      ...
      p(
        "Ce lien ouvre l'écran où vous saisirez votre nouveau mot de passe. Il ne sert qu'une fois.",
      ),
      button("Choisir un nouveau mot de passe", "{{ .ConfirmationURL }}"),
...
/** The generated module: the invitation's HTML, and nothing else to keep in step by hand. */
...
// The invitation e-mail the app sends itself (D75), identical to supabase/templates/invite.html.
// Regenerate with: pnpm gen:emails

export const INVITATION_SUBJECT = ${JSON.stringify(invite.subject)};

export const INVITATION_HTML = ${JSON.stringify(render(invite))};
`;
}
...
function main() {
  ...
  if (check) {
    if (readFileSync(APP_TEMPLATE, "utf8") !== appTemplate) {
      console.error("stale: src/lib/email/invitation.generated.ts");
      stale += 1;
    }
  } else {
    writeFileSync(APP_TEMPLATE, appTemplate);
    console.log("wrote src/lib/email/invitation.generated.ts");
  }
