...
/**
 * The password panel: the same visual pattern as `code()` (monospace, boxed, nothing competing
 * with it), for `{{ .Data.password }}` rather than `{{ .Token }}` — 8 digits, so the letter
 * spacing is tuned a little tighter to stay comfortably inside the panel on a narrow phone.
 */
const passwordPanel =
  () => `<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:24px 0 0">
  <tr>
    <td class="x-panel" align="center" bgcolor="${C.muted}" style="background-color:${C.muted};border:1px solid ${C.border};border-radius:12px;padding:20px 16px">
      <div class="x-ink" style="font-family:${MONO};font-size:28px;line-height:1.2;font-weight:700;letter-spacing:0.18em;text-indent:0.18em;color:${C.ink}">{{ .Data.password }}</div>
    </td>
  </tr>
</table>`;
...
/**
 * E-mails only the app sends (D91): no `supabase/templates/` file, no `config.toml` block — Auth
 * never sends them. Same shell, same placeholders, resolved by `src/lib/email/inbox.ts`.
 */
export const APP_ONLY_TEMPLATES = [
  /**
   * D151: the whole invitation flow, in one e-mail. No token, no link, no code — the account and
   * the membership already exist by the time this is sent; signing in with this address and this
   * password is all that is left to do. Sent again, unchanged in shape, whenever the owner
   * "relance" someone: a fresh password each time, same message.
   */
  {
    key: "credentials",
    subject: "Votre accès Xaman{{ if .Data.boat_name }} — {{ .Data.boat_name }}{{ end }}",
    title: "Votre accès Xaman",
    preheader: "Votre adresse et votre mot de passe pour vous connecter.",
    body: [
      h1("Votre accès Xaman"),
      p(
        `{{ if .Data.inviter_name }}${strong("{{ .Data.inviter_name }}")} vous donne accès{{ else }}Vous avez accès{{ end }} au carnet d'entretien{{ if .Data.boat_name }} de ${strong("{{ .Data.boat_name }}")}{{ end }}. Voici de quoi vous connecter dès maintenant.`,
      ),
      facts([
        ["Identifiant", "{{ .Data.email }}"],
        ["Votre rôle", "{{ if .Data.role_label }}{{ .Data.role_label }}{{ else }}Membre{{ end }}"],
      ]),
      p("Mot de passe", { color: C.ink3, size: 13, top: 24 }),
      passwordPanel(),
      p(
        "Vous pouvez vous connecter dès maintenant avec cette adresse et ce mot de passe. Vous pourrez en choisir un autre depuis votre profil, une fois connecté.",
        { color: C.ink3, size: 13, top: 20 },
      ),
      button("Se connecter à Xaman", "{{ .Data.login_url }}"),
      fallbackLink("{{ .Data.login_url }}"),
    ],
  },
  {
    key: "inbox_ready",
    subject: "Un document attend votre validation — {{ .Data.boat_name }}",
    title: "Document à valider",
    preheader: "Un document est arrivé sur le carnet, il attend votre validation.",
    body: [
      h1("Un document est arrivé"),
      p(
        `${strong("{{ .Data.sender }}")} a envoyé {{ .Data.count_label }} sur le carnet de ${strong("{{ .Data.boat_name }}")}. L'analyse a préparé une intervention ou un achat : il reste à vérifier les informations et à valider.`,
      ),
      facts([
        ["Bateau", "{{ .Data.boat_name }}"],
        ["De", "{{ .Data.sender }}"],
        ["Objet", "{{ if .Data.subject }}{{ .Data.subject }}{{ else }}Sans objet{{ end }}"],
      ]),
      button("Valider dans Xaman", "{{ .Data.inbox_url }}"),
      p(
        "Rien n'est écrit dans le carnet tant que personne n'a validé : ce document attend sur l'écran « À valider ».",
        { color: C.ink3, size: 13, top: 20 },
      ),
      fallbackLink("{{ .Data.inbox_url }}"),
    ],
  },
  {
    key: "inbox_validated",
    subject: "{{ .Data.kind_label }} validée — {{ .Data.boat_name }}",
    title: "Validé",
    preheader: "Un document a été validé et inscrit dans le carnet.",
    body: [
      h1("C'est dans le carnet"),
      p(
        `${strong("{{ .Data.validator_name }}")} a validé un document reçu sur le carnet de ${strong("{{ .Data.boat_name }}")}.`,
      ),
      facts([
        ["{{ .Data.kind_label }}", "{{ .Data.title }}"],
        ["Date", "{{ .Data.date_label }}"],
        [
          "Montant",
          "{{ if .Data.amount_label }}{{ .Data.amount_label }}{{ else }}Non renseigné{{ end }}",
        ],
      ]),
      button("Voir dans le carnet", "{{ .Data.url }}"),
      fallbackLink("{{ .Data.url }}"),
    ],
  },
  /**
   * « Confier au chantier » (D141): the yard receives a maintenance point with everything it
   * needs to answer — which boat, which system, how late, at what hours, when it was last done,
   * and who to answer. No button: the yard has no account, and needs none; the reply is a
   * reply. The last line is the one door the e-mail opens for a yard that does not know Xaman.
   */
  {
    key: "handoff",
    subject: "Demande d'intervention — {{ .Data.boat_name }} · {{ .Data.item_label }}",
    title: "Demande d'intervention",
    preheader:
      "Un propriétaire vous confie un entretien de son bateau, avec ce qu'il faut pour répondre.",
    body: [
      h1("Un entretien vous est confié"),
      p(
        `${strong("{{ .Data.requester_name }}")} vous confie ${strong("{{ .Data.item_label }}")} sur le carnet de ${strong("{{ .Data.boat_name }}")}. Tout ce qu'il faut pour répondre est ci-dessous ; la réponse se fait par retour de mail.`,
      ),
      facts([
        [
          "Bateau",
          "{{ .Data.boat_name }}{{ if .Data.boat_model }} · {{ .Data.boat_model }}{{ end }}",
        ],
        [
          "Système",
          "{{ if .Data.category_name }}{{ .Data.category_name }}{{ else }}Non renseigné{{ end }}",
        ],
        ["Échéance", "{{ .Data.due_label }}"],
        [
          "Heures moteur",
          "{{ if .Data.hours_label }}{{ .Data.hours_label }}{{ else }}Compteur non relevé{{ end }}",
        ],
        [
          "Dernière fois",
          "{{ if .Data.last_done_label }}{{ .Data.last_done_label }}{{ else }}Jamais noté dans le carnet{{ end }}",
        ],
        ["Message", "{{ if .Data.message }}{{ .Data.message }}{{ else }}—{{ end }}"],
      ]),
      p(
        `Pour répondre : ${strong("{{ .Data.requester_name }}")} — ${link("{{ .Data.requester_email }}", "mailto:{{ .Data.requester_email }}")}.`,
        { top: 24 },
      ),
      p(
        "Dans le carnet, l'intervention est notée « planifiée » sur ce point : elle sera cochée quand elle sera faite, avec ses heures moteur et sa facture.",
        { color: C.ink3, size: 13, top: 20 },
      ),
      p(
        `Xaman est le carnet d'entretien numérique du bateau. Vous êtes un chantier ? ${link("Ce que Xaman propose aux constructeurs", "{{ .SiteURL }}/constructeurs")}.`,
        { color: C.ink3, size: 13, top: 12 },
      ),
    ],
  },
];
...
/** The generated module: the e-mails the app sends, and nothing to keep in step by hand. */
export function renderAppTemplate() {
  const names = {
    ...
    inbox_validated: "INBOX_VALIDATED",
    handoff: "HANDOFF",
  };
  const blocks = APP_TEMPLATE_KEYS.map((key) => {
    const template = [...TEMPLATES, ...APP_ONLY_TEMPLATES].find((t) => t.key === key);
...
// The e-mails the app sends itself: the invitation (D75) and the password-recovery code (D78),
// identical to their supabase/templates/*.html counterparts, the credentials e-mail (D151), the
// two e-mails of the inbox (D91) and the hand-over to the yard (D141) — these three only the app
// sends.
